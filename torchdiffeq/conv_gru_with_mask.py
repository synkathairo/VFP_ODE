...
def save_frames(batch:torch.tensor, 
        ...
        for frame in range(window_size): 
            ...
            cv2.imwrite(
                f'/Users/ssarch/Documents/acads/sem-1/Image&video_process/proj/torchdiffeq/images/{name}{frame}_non_proc.png',
                save_img2) 
...
def main_worker(gpu, args): 
	...
	func = torch.nn.parallel.DistributedDataParallel(func, device_ids = [gpu], find_unused_parameters=True)

    conv_enc = ConvEncoder().to(args.rank)
	conv_enc = torch.nn.parallel.DistributedDataParallel(conv_enc, device_ids = [gpu], find_unused_parameters=True)
    conv_dec = ConvDecoder(inp_dim=3).to(args.rank)
	conv_dec = torch.nn.parallel.DistributedDataParallel(conv_dec, device_ids = [gpu], find_unused_parameters=True)
	...
	sampler = torch.utils.data.distributed.DistributedSampler(mmnist_train)

	train_loader = torch.utils.data.DataLoader(
			dataset=mmnist_train, batch_size=args.batch_size, num_workers=args.workers,
			pin_memory=True, sampler=sampler,  collate_fn = MyCollate(nframes=args.nframes, frame_size=args.frame_size))
